...
- Confirm the public repo exists at `zerkerlabs/zerker-memory`.
- Verify the live raw installer URL from a clean shell:
...
zmem prelaunch --allow-placeholders
```
...
If isolated editable install cannot fetch packaging dependencies, the smoke retries with `--no-build-isolation` and then falls back to local venv wrapper entrypoints so the rest of the release proof can still run in restricted environments.
...
Use `zmem prelaunch --allow-placeholders` before the final repo URL is chosen, then plain `zmem prelaunch` after replacing placeholders and before tagging. The prelaunch gate now expects both launch-proof and handoff artifacts to exist.
