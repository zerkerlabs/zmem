...
- **Local SQLite store:** best for read-only inspection, exports, or dashboards. Route writes through ZMem so receipts and lifecycle rules are preserved.
...
zmem --db .zerker/imported.sqlite restore --handoff-dir .zerker/handoff --dry-run --summary-only
zmem --db .zerker/imported.sqlite restore --handoff-dir .zerker/handoff --confirm-preview <preview-id>
```
...
Do not write directly to ZMem tables. The schema is inspectable, but direct writes bypass event-chain, receipt, policy, and lifecycle behavior.
