...
ZMem has four complete `1,986`-question local retrieval-recall artifacts over the same SHA-pinned LoCoMo conversion:
...
A one-mode matrix records the verified result once; it does not compare the result to itself. Summary-only output bounds long question-id lists to ten examples plus an omitted count.
...
- LoCoMo same-commit matrix: confirm mode deltas after removing single-mode comparison duplication. `zmem-retrieval` is the pseudo-rerank alias, not a separate full run.
