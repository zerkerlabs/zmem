...
ZMem has two complete `1,986`-question local retrieval-recall artifacts over the same SHA-pinned LoCoMo conversion:
...
The multihop run improved overall, single-hop, multi-hop, open-domain, and token use, while temporal accuracy declined. That is useful engineering evidence, not a leaderboard win.
...
- LoCoMo `pseudo-embedding` and `pseudo-embedding-rerank`: measure dense recall and reranking in one isolated matrix.
