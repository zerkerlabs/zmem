...
To make one current connector visible as a named live session, create a short-lived invitation:

```bash
zmem session invite --agent codex --label release-chat --summary-only
```

Paste the printed instruction into that chat. The agent calls `memory.session_attach` with the one-time code. Then inspect every live, idle, or detached attachment:
