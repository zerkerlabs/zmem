...
To make one current connector visible as a named live session, open `zmem ui` and use **Agent Memory Network**. Choose the agent, label the chat, optionally add Room context, then create and copy the one-time instruction.

The same operation is available in the terminal:
