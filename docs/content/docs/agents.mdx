...
To connect current agent sessions, run:

```bash
cd your-project
zmem connect codex --label current-chat

# One workspace, one separate invitation per current agent.
zmem connect codex claude-code hermes --label release-work
```

ZMem initializes the workspace once, configures every requested agent host, and prints a separate copy-ready `memory.session_attach` instruction for each one. List every host before command options. Reload a client if it has not picked up the MCP entry, then paste only its named instruction into that chat. Invitations are bound to the configured agent identity, single-use, and expire after ten minutes by default. If any host cannot be configured, ZMem issues no invitations.

For config-only setup without creating current-session invitations, use:
